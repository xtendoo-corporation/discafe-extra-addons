...
class SalePromotion(models.Model):
    ...
    apply_on_product_tmpl_ids = fields.Many2many(
        ...
        relation='sale_promotion_apply_on_product_tmpl_rel')
    apply_on_product_ids = fields.Many2many(
        comodel_name='product.product',
        string='Products',
        relation='sale_promotion_apply_on_product_rel')
    apply_on_category_ids = fields.Many2many(
        comodel_name='product.category',
        string='Category',
        relation='sale_promotion_apply_on_category_rel')
    ...
    promotion_product_tmpl_ids = fields.Many2many(
        comodel_name='product.template',
        string='Product Templates',
        relation='sale_promotion_promotion_product_tmpl_rel')
    promotion_product_ids = fields.Many2many(
        comodel_name='product.product',
        string='Products',
        relation='sale_promotion_promotion_product_rel')
    promotion_category_ids = fields.Many2many(
        comodel_name='product.category',
        string='Category',
        relation='sale_promotion_promotion_category_rel')
    ...
    def get_apply_to_order_lines(self, order_lines):
        ...
        if self.apply_to == 'product_template':
            return order_lines.filtered(
                lambda l: l.product_id.product_tmpl_id in self.apply_on_product_tmpl_ids)
        elif self.apply_to == 'product_variant':
            return order_lines.filtered(
                lambda l: l.product_id in self.apply_on_product_ids)
        elif self.apply_to == 'product_category':
            return order_lines.filtered(
                lambda l: l.product_id.product_categ_id in self.apply_on_category_ids)
